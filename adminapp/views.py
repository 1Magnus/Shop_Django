...
from authapp.forms import ShopUserRegisterForms
...
def user_create(request):
    if request.method == 'POST':
        user_form = ShopUserRegisterForms(request.POST, request.FILES)
        if user_form.is_valid():
            user_form.save()
            return HttpResponseRedirect(reverse('adminapp:user_list'))
    else:
        user_form = ShopUserRegisterForms()
...
class ProductDeleteView(AccessMixin, DeleteView):
    ...
    #     if self.object.is_acrive:
    #         self.object.is_acrive = False
    #     else:
    #         self.object.is_acrive = True
    #     self.object.save()
...
class ProductDetailView(AccessMixin, DetailView):
    model = Product
    template_name = 'adminapp/products_detail.html'
